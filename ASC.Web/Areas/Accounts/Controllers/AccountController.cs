...
namespace ASC.Web.Areas.Accounts.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> ServiceEngineers()
        {
            ...
            {
                SericeEngineers = serviceEngineers == null ? null : serviceEngineers.ToList(),
        ...
        {
            serviceEngineer.SericeEngineers = HttpContext.Session.GetSession<List<IdentityUser>>("ServiceEngineers");
            ...
            if (serviceEngineer.Registration.IsEdit)
            {
                ...
                user.UserName = serviceEngineer.Registration.Email;
        ...
        public async Task<IActionResult> Customers(CustomerViewModel model)
        {
            ...
            if (model.Registration.IsEdit)
            {
                ...
                var isActiveClaim = identity.SingleOrDefault(p => p.Type == "IsActive");
                var removeClaimResult = await _userManager.RemoveClaimAsync(user, new System.Security.Claims.Claim(isActiveClaim.Type, isActiveClaim.Value));
                var addClaimResult = await _userManager.AddClaimAsync(user, new System.Security.Claims.Claim("IsActive", model.Registration.IsActive.ToString()));

                if (model.Registration.IsActive)
                {
                    await _emailSender.SendEmailAsync(model.Registration.Email, "Account Modified", $"Your account has been activated, Email : {model.Registration.Email}");
                }
                else
                {
                    await _emailSender.SendEmailAsync(model.Registration.Email, "Account Deactivated", $"Your account has been deactivated.");
                }
            }
